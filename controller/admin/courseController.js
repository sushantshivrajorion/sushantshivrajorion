/**
 * courseController.js
 * @description : exports action methods for course.
 */
...
/**
 * @description : create multiple documents of Course in mongodb collection.
 * @param {Object} req : request including body for creating documents.
 * @param {Object} res : response of created documents.
 * @return {Object} : created Courses. {status, message, data}
 */
const bulkInsertCourse = async (req,res)=>{
  try {
    ...
    let createdCourses = await dbService.create(Course,dataToCreate);
    createdCourses = { count: createdCourses ? createdCourses.length : 0 };
    return res.success({ data:{ count:createdCourses.count || 0 } });
  } catch (error){
    return res.internalServerError({ message:error.message });
  }
};
...
/**
 * @description : update multiple records of Course with data by filter.
 * @param {Object} req : request including filter and data in request body.
 * @param {Object} res : response of updated Courses.
 * @return {Object} : updated Courses. {status, message, data}
 */
const bulkUpdateCourse = async (req,res)=>{
  try {
    ...
    let updatedCourse = await dbService.updateMany(Course,filter,dataToUpdate);
    if (!updatedCourse){
      return res.recordNotFound();
    }
    return res.success({ data :{ count : updatedCourse } });
  } catch (error){
    return res.internalServerError({ message:error.message }); 
  }
};
...
/**
 * @description : partially update document of Course with data by id;
 * @param {obj} req : request including id in request params and data in request body.
 * @param {obj} res : response of updated Course.
 * @return {obj} : updated Course. {status, message, data}
 */
...
const softDeleteCourse = async (req,res) => {
  try {
    ...
    let updatedCourse = await deleteDependentService.softDeleteCourse(query, updateBody);
    if (!updatedCourse){
      return res.recordNotFound();
    }
    return res.success({ data:updatedCourse });
...
const deleteCourse = async (req,res) => {
  try {
    ...
    if (req.body.isWarning) { 
      deletedCourse = await deleteDependentService.countCourse(query);
    } else {
      deletedCourse = await deleteDependentService.deleteCourse(query);
    }
    if (!deletedCourse){
      return res.recordNotFound();
    }
...
/**
 * @description : deactivate multiple documents of Course from table by ids;
 * @param {Object} req : request including array of ids in request body.
 * @param {Object} res : response contains updated documents of Course.
 * @return {Object} : number of deactivated documents of Course. {status, message, data}
 */
const softDeleteManyCourse = async (req,res) => {
  try {
    let ids = req.body.ids;
    if (!ids || !Array.isArray(ids) || ids.length < 1) {
      return res.badRequest();
